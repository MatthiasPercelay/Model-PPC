...
int fixedWork_[i in AGENTS][j in DAYS] = (fixedWork[i][j] == 1 || timetable[i][j] == "W") && (timetable[i][j] != "FO" && timetable[i][j] != "EX");
...
subject to{
	...
	forall(j in DAYS, i in AGENTS){
		...
		if(fixedShift[i][j] == 1) work[i][j] == SHIFT[timetable[i][j]];
		// If an agent must work a certain DAY, then he must have a shift
		if(fixedWork_[i][j] == 1 && timetable[i][j] != "FO") work[i][j] != 0;
		if(fixedWork_[i][j] == 0 || timetable[i][j] == "FO") work[i][j] == 0;
	}
